...
export class Post {
  ...
  static async allPostInDashboard(page:number, per_page: number) {
    ...
    if (response.status == 200) {
      ...
      page_data.current_page = response.data.posts.current_page;
      page_data.last_page = response.data.posts.last_page;

      response.data.posts.data.forEach((post) => {
        posts.push({
          id: post.id,
          img: 'http://127.0.0.1:8000/' + post.media[0].url,
      ...
      return {
        posts: posts,
        page_data: page_data,
        top_post: response.data.topPosts,
      };
    }
    throw Error('Deleted Failed');
  }

...
const userData =  () => {
  ...
  .then(
    (value) => {
      ...
      profileTemp.value.id = value.data.user.id;
      profile.value.username = value.data.user.name;
      profileTemp.value.username = value.data.user.name;
      profile.value.email = value.data.user.email;
      profileTemp.value.email = value.data.user.email;
      if(value.data.user.media[0]?.url){
        profile.value.avatar = serverRoute + value.data.user.media[0]?.url;
        profileTemp.value.avatar = serverRoute + value.data.user.media[0]?.url;
      }
      profile.value.role = value.data.user.roles[0].name
      profileTemp.value.role = value.data.user.roles[0].name
    }
  )
}
userData();
...
export {profile, profileTemp, userData}
